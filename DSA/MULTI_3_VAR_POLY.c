...
struct node *PolyMultiply(struct node *head1, struct node *head2)
{
    ...
    while (ptr1 != NULL)
    {
        ...
        while (ptr2 != NULL)
        {
            ...
            int expX = ptr1->expX + ptr2->expX;
            int expY = ptr1->expY + ptr2->expY;
            int expZ = ptr1->expZ + ptr2->expZ;

            head3 = insert(head3, coeff, expX, expY, expZ);
...
int main()
{
    ...
    result = PolyMultiply(head1, head2);
